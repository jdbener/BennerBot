package me.jdbener.plugins.moderataion;
...
public class GeneralFilter extends ListenerAdapter<PircBotX>{
	...
	public void onMessage(MessageEvent<PircBotX> e){
		MaxMessageLength = Integer.parseInt(Bennerbot.conf.get("MaxMessageLength").toString());
		boolean filter = false;
		
		if(e.getMessage().length() > MaxMessageLength && filter == false && Bennerbot.conf.get("LengthFilter").toString().equalsIgnoreCase("true")){
